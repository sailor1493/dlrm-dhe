...
class EmbeddingBagCollection(EmbeddingBagCollectionInterface):
    ...
    * B: batch size
    ...
    def _create_embedding(self, mode, embedding_config, device):
        dtype = (
            torch.float32
        ...
        pooling = pooling_type_to_str(embedding_config.pooling)
        if pooling == "sum":
            op = "add"
        if mode == "qre":
            return QREmbeddingBag(
                num_categories=embedding_config.num_embeddings,
                embedding_dim=embedding_config.embedding_dim,
                num_collisions=256,
                operation=op,
                mode=pooling,
    ...
    def __init__(
        ...
        for embedding_config in tables:
            ...
            # HERE
            self.embedding_bags[embedding_config.name] = self._create_embedding(
                mode, embedding_config, device
            )
